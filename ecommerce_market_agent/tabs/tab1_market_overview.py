...
def _filter(df: pd.DataFrame, filters: dict, apply_category: bool = True) -> pd.DataFrame:
    """날짜 범위 + 중분류 기준으로 데이터프레임을 필터링합니다."""
    if df.empty:
        return df
    if "date" in df.columns:
        df = df[(df["date"] >= filters["date_start"]) & (df["date"] <= filters["date_end"])]
    if apply_category and "category_name" in df.columns and filters.get("mid_cats"):
        df = df[df["category_name"].isin(filters["mid_cats"])]
    return df.copy()
...
def render(data: dict[str, pd.DataFrame], filters: dict) -> None:
    ...
    st.caption(f"📅 조회 기간: {ds} ~ {de}  |  중분류: {', '.join(filters['mid_cats']) if filters['mid_cats'] else '전체'}")
    ...
    with c1:
        if not cat_df.empty:
            top_cat = cat_df.groupby("category_name")["ratio"].mean().idxmax()
            st.metric("인기 카테고리", top_cat)
        else:
            st.metric("인기 카테고리", "-")
    with c2:
        if not kw_df_f.empty:
            top_kw = kw_df_f.groupby("keyword")["ratio"].mean().idxmax()
    ...
    with c3:
        if not dev_df.empty:
            mobile_ratio = (
                dev_df[dev_df["device"] == "mo"]["ratio"].mean()
                / dev_df["ratio"].mean() * 100
            )
            st.metric("모바일 비중", f"{mobile_ratio:.1f}%")
        else:
            st.metric("모바일 비중", "-")
    with c4:
        yr, (w0, w1) = filters["year"], filters["week_range"]
        st.metric("분석 기간", f"{yr}년 {w0}~{w1}주")
    ...
    with col1:
        ...
        if not dev_df.empty:
            agg_dev = (
                dev_df.groupby(["category_name", "device"])["ratio"]
                .mean().reset_index()
            )
            agg_dev["device"] = agg_dev["device"].map({"pc": "PC", "mo": "모바일"})
            fig_dev = px.bar(
                agg_dev,
                x="category_name", y="ratio", color="device", barmode="group",
    ...
    with col2:
        ...
        if not gen_df.empty:
            agg_gen = (
                gen_df.groupby(["category_name", "gender"])["ratio"]
                .mean().reset_index()
            )
            agg_gen["gender"] = agg_gen["gender"].map({"m": "남성", "f": "여성"})
            fig_gen = px.bar(
                agg_gen,
                x="category_name", y="ratio", color="gender", barmode="group",
    ...
    if not age_df.empty:
        ...
        agg_age = (
            age_df.groupby(["category_name", "age_group"])["ratio"]
            .mean().reset_index()
        )
        agg_age["age_label"] = agg_age["age_group"].map(AGE_LABEL)
        agg_age["age_label"] = pd.Categorical(agg_age["age_label"], categories=AGE_ORDER, ordered=True)
        agg_age = agg_age.sort_values("age_label")

        fig_age = px.bar(
            agg_age,
            x="age_label", y="ratio", color="category_name",
            barmode="group",
    ...
    if not kw_df_raw.empty:
        # 소분류 필터 적용 (선택된 경우에만)
        if filters.get("small_cats"):
            kw_df_raw = kw_df_raw[kw_df_raw["keyword"].isin(filters["small_cats"])]

        agg_kw = (
            kw_df_raw.groupby(["keyword", "category_name"])["ratio"]
            .mean().reset_index()
        )
        top10 = (
        ...
        # 카테고리 정보 재결합
        top10_detail = agg_kw[agg_kw["keyword"].isin(top10["keyword"])]
